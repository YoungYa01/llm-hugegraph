...
import { AppButton, AppSelect, AppTextArea } from "../components/SemiAdapter.jsx";
...
function selectedHypothesis(hypotheses, decision) {
	...
	if (String(decision?.source || "").toLowerCase() === "llm" && decision?.selected_node_id && modelChain.length)
		return {
...
function normalizedDisplayChain(value, algorithmChain) {
	...
	const byNode = new Map(
		value.filter((item) => item && typeof item === "object").map((item) => [String(item.node || ""), item]),
	);
	...
	return algorithmChain.map((node, index) => {
		...
		return {
			...
			stage: String(
				item.stage || (index === 0 ? "根因" : index === algorithmChain.length - 1 ? "受影响入口" : "故障传播"),
			),
		};
	});
}
function normalizedReasonItems(value, fallback) {
	const values = Array.isArray(value) && value.length ? value : String(fallback || "").split(/(?:\r?\n|[；;])/);
	return [
		...new Set(
			values
				.map((item) =>
...
function formatEvidenceItem(item) {
	...
	const header = [item.timestamp, item.level ? `[${item.level}]` : "", item.service || item.service_name]
		.filter(Boolean)
		.join(" ");
...
function OrderedReasons({ items }) {
	return items.length ? (
		...
		<p style={{ margin: 0, color: "var(--ink-500)" }}>暂无可展示的最可能原因。</p>
	);
}
function Chain({ chain, expanded, onToggle }) {
	...
	const visible = shouldFold ? [chain[0], chain[1], { __fold: chain.length - 4 }, chain.at(-2), chain.at(-1)] : chain;
	return (
		<>
			<div
					...
					return (
						<React.Fragment key={`${item.node}:${index}`}>
							...
							>
								...
								>
									<strong style={{ display: "block" }}>{item.label || item.node}</strong>
									{item.explanation ? (
										<small
											style={{
												display: "block",
...
export function IncidentDetailPage({ project, incidentId }) {
	...
	const model = useMemo(() => {
		...
		const possibleMembers = Array.isArray(llmDecision.possible_member_nodes) ? llmDecision.possible_member_nodes : [];
		...
		const supporting = hypothesisForEvidence(hypotheses, llmDecision) || algorithmTop;
		const evidenceNotes = [
			...new Set(
				[
		...
		return {
			...
			rootRuntimeTarget: runtimeTargetSummary(llmDecision.selected_node_runtime),
			showMemberResolution: possibleMembers.length > 0 || llmDecision.instance_resolution === "members_missing",
		};
	}, [incident]);
	const visibleGraph = useMemo(() => filterIncidentGraph(fusionGraph, false), [fusionGraph]);
	useEffect(() => {
		if (!incident || !model || !canvasRef.current || !visibleGraph.nodes.length) return undefined;
	...
	async function saveStatus(event) {
		...
		try {
			...
			const res = await api.updateIncidentStatus(project.id, incident.id, payload);
	...
	if (error || !incident || !model) return <ErrorState error={error || "故障不存在"} onRetry={load} />;
	...
	return (
		<>
			<div className="page-header">
				...
				<div className="page-actions" style={{ display: "flex", alignItems: "center", gap: 8 }}></div>
			</div>
			<section
			...
			>
				<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="hero-grid-responsive">
					<div>
						<div
							style={{
								display: "flex",
						...
						>
							...
							{llmConfidence ? (
								<div style={{ display: "flex", gap: 10 }}>
									<span className="badge badge-subtle" style={{ fontWeight: 600, color: "var(--brand)" }}>
						...
						>
							...
							>
								{top.fault_mode || llmDecision.selected_fault_mode || "故障未知"}
							</span>
						</div>
						{rootRuntimeTarget ? (
							<div
						...
						>
							<span className="badge">来源: {llmDecision.source || "RCA 推理引擎"}</span>
							...
							<span className="badge">
								日志侧定位: {model.analysis.resolved_root_service || detail.root_service_candidate || "未知服务"}
							</span>
						</div>
					</div>
					<div style={{ borderLeft: "1px solid var(--border)", paddingLeft: 24 }} className="hero-steps-border">
						<span
							className="stat-label stat-label-with-icon"
							style={{
								fontSize: 12,
						...
						{llmSteps.length ? (
							...
							<p style={{ color: "var(--ink-500)", fontSize: 13 }}>暂无自动推荐的处置步骤，请核查下方证据与日志链。</p>
						)}
					</div>
				</div>
			</section>
			{showMemberResolution ? (
				<section className="card" style={{ marginBottom: 20, borderColor: "rgba(217,119,6,0.3)" }}>
					<div
					...
					>
						...
						>
							{possibleMembers.length ? `${possibleMembers.length} 个真实成员` : "成员关系缺失"}
						</span>
					</div>
										...
										>
											...
											>
												<div style={{ minWidth: 0 }}>
													...
													<span style={{ fontSize: 12, color: "var(--ink-500)" }}>{member?.kind || "Instance"}</span>
												</div>
												<span
													className="badge"
													style={{
														background: direct ? "rgba(220,38,38,0.1)" : "rgba(217,119,6,0.1)",
											...
											>
												{member?.reason || "属于异常集群，当前缺少实例级直接证据。"}
											</p>
										</article>
									);
								})}
						...
						<div className="incident-graph-layout">
							<div className="graph-shell graph-shell-fusion">
								...
								<div className="graph-legend">
									<span>
										<i className="legend-dot" style={{ background: "#c44578" }} />
										界面交互
									</span>
									<span>
										<i className="legend-dot" style={{ background: "#3f6fba" }} />
										服务
									</span>
									<span>
										<i className="legend-dot" style={{ background: "#178a80" }} />
										数据资源
									</span>
									<span>
										<i className="legend-dot" style={{ background: "#117b74" }} />
										集群
									</span>
									<span>
										<i className="legend-dot" style={{ background: "#c8722f" }} />
										实例
									</span>
									<span>
										<i
							...
							<div className="incident-graph-aside">
								<section className="incident-graph-panel incident-chain-panel">
									...
									<div className="incident-chain-scroll">
										<Chain chain={chain} expanded={chainExpanded} onToggle={() => setChainExpanded((x) => !x)} />
									</div>
								</section>
										...
										{!selection ? (
											...
											>
												...
												>
													...
													>
														{selection.friendly?.stage || selection.node.kind || "Node"}
													</span>
												</div>
												{selection.friendly?.label && selection.friendly.label !== selection.node.name ? (
													<div style={{ color: "var(--ink-500)", fontSize: 11 }}>真实节点：{selection.node.name}</div>
												) : null}
												...
												>
													<span>相邻关联实体:</span>
													<strong style={{ color: "var(--ink-800)" }}>{selection.edges.length} 个关系</strong>
												</div>
											</div>
										) : (
											<div
											...
											>
												...
												<div
													style={{
														marginTop: 4,
														color: selection.edge.description ? "var(--ink-700)" : "var(--ink-500)",
														lineHeight: 1.4,
														fontStyle: selection.edge.description ? undefined : "italic",
													}}
												>
			...
			<BackTop className="incident-detail-backtop" visibilityHeight={360} duration={320} aria-label="回到页面顶部">
				<IconArrowUp />
			</BackTop>
					...
					<section className="card">
						<div className="card-header">
							<div>
								...
								<p>按根因描述合并连续重复事件（优先 root_cause，间隔不超过 1 分钟），包含根因日志的分组高亮展示，展开可查看各服务、实例的原始日志。</p>
							</div>
						</div>
						<div className="card-body">
							<IncidentTimeline items={detail.timeline || []} faultStart={detail.fault_start} />
						</div>
					</section>
				</div>
						...
						<div className="card-body">
							<form className="form-stack" onSubmit={saveStatus}>
								...
								<div className="field">
									...
									/>
									<span className="field-hint">标记为“已解决”时，建议记录具体的故障原因与修复处理措施。</span>
								</div>
								<AppButton className="button button-primary" type="submit" style={{ width: "100%" }} disabled={saving}>
									{saving ? "保存中…" : "保存处理结果"}
								</AppButton>
							</form>
						</div>
						...
						<div className="card-body">
							<dl className="kv-list">
								...
								<div className="kv-row">
									<dt>异常模式/类型</dt>
									<dd>
										<code style={{ fontSize: 12 }}>{detail.root_cause_candidate || "—"}</code>
									</dd>
								</div>
								<div className="kv-row">
									<dt>关联 TraceId</dt>
								...
								<div className="timeline">
									{incident.actions.map((item, i) => (
										<div className="timeline-item" key={i}>
											<time>
												{formatDate(item.created_at)} · {item.display_name || item.username}
											</time>
											<p>

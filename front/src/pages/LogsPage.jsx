...
function durationText(item) {
	...
	return Number(seconds) >= 60
		? `${Math.floor(Number(seconds) / 60)}分 ${Math.round(Number(seconds) % 60)}秒`
...
export function LogsPage({ project }) {
	...
	useEffect(() => {
		const onDone = (event) => {
			if (event.detail?.type === "logs") load();
		};
	...
	const nameCounts = useMemo(
		() =>
			batches.reduce(
				(acc, item) => ({
					...acc,
					[item.filename]: (acc[item.filename] || 0) + 1,
				}),
				{},
			),
		[batches],
		...
		() => [
			{
				...
				render: (_, item) => (
					<div>
						<span className="table-title">{item.filename}</span>
						{nameCounts[item.filename] > 1 ? (
							<span className="table-subtitle">{formatDate(item.created_at)}</span>
						) : null}
						{item.train_filename ? <span className="table-subtitle">训练集：{item.train_filename}</span> : null}
						{item.error_message ? (
			...
			},
			{
				...
				render: (_, item) => (
					<>
						<strong>{item.summary?.events ?? "—"} 条</strong> {/*<small style={{ color: "var(--ink-500)" }}>*/}
						{/*  / {item.summary?.windows ?? "—"} 窗口*/}
						{/*</small>*/}
					</>
				),
			},
			{
				title: "挖掘异常段数",
				render: (_, item) =>
					item.summary?.incidents != null ? (
						<Link
							to={paths.incidents(project.id, { batch: item.id })}
							style={{ fontWeight: 700, color: "var(--brand)", fontSize: 13 }}
						>
							{item.summary.incidents} 段
						</Link>
					) : (
						"—"
					),
			},
			{
				title: "分析时长",
				render: (_, item) => (
					<span className="table-subtitle" style={{ fontWeight: 600, color: "var(--ink-700)" }}>
						{durationText(item)}
					</span>
				),
			},
			{
				title: "解析完成时间",
				render: (_, item) => formatDate(item.completed_at || item.created_at),
			},
			{
				title: "操作",
				render: (_, item) => (
		...
		<>
			...
			<section className="card" style={{ marginBottom: 24 }}>
				...
				<div className="card-body">
					...
					<form className="form-stack" onSubmit={submit}>
						...
						>
							<div className="notice notice-info" style={{ margin: 0, flex: 1, minWidth: 280 }}>
			...
			<section className="card">
				...
				<div className="card-body flush">
					{!batches.length ? (
						<EmptyState title="还没有日志批次" detail="上传 Spring 日志后，分析记录会出现在这里。" />
					) : (
						<Table

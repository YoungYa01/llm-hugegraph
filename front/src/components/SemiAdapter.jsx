...
import { Button as SemiButton, Checkbox, Input, Select, TextArea } from "@douyinfe/semi-ui";
...
export function AppButton({ type = "button", className = "", children, ...props }) {
  ...
  return <SemiButton {...visual} {...props} htmlType={htmlType} className={className}>{children}</SemiButton>;
}

function useCompatValue(value, defaultValue = "") {
  const controlled = value !== undefined;
  const [inner, setInner] = useState(defaultValue ?? "");
  useEffect(() => { if (controlled) setInner(value ?? ""); }, [controlled, value]);
...
export function AppInput({ onChange, className = "input", name, value, defaultValue, type, ...props }) {
  ...
  return <>
    <Input {...props} mode={mode} className={className} value={current} onChange={(next) => { setCurrent(next); onChange?.({ target: { value: next } }); }} />
...
export function AppTextArea({ onChange, className = "input", name, value, defaultValue, ...props }) {
  const [current, setCurrent] = useCompatValue(value, defaultValue);
  return <>
    <TextArea {...props} className={className} value={current} onChange={(next) => { setCurrent(next); onChange?.({ target: { value: next } }); }} />
    {name ? <input type="hidden" name={name} value={current} /> : null}
  </>;
}
...
export function AppSelect({ children, onChange, className = "select", name, value, defaultValue, ...props }) {
  const options = useMemo(() => React.Children.toArray(children)
    ...
    .map((child) => ({ value: child.props.value, label: child.props.children, disabled: child.props.disabled })), [children]);
  const fallback = defaultValue ?? options[0]?.value ?? "";
  const [current, setCurrent] = useCompatValue(value, fallback);
  return <>
    <Select {...props} className={className} value={current} optionList={options} onChange={(next) => { setCurrent(next); onChange?.({ target: { value: next } }); }} />
    {name ? <input type="hidden" name={name} value={current} /> : null}
  </>;
}
...
export function AppCheckbox({ checked, defaultChecked, onChange, name, value = "on", ...props }) {
  ...
  return <>
    <Checkbox {...props} checked={current} onChange={(event) => {

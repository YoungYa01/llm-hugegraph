...
export function buildNavigationModel({ project = null, current = "projects", isAdmin = false } = {}) {
  const sections = [
    {
      key: "workspace",
      label: "项目空间",
      items: [{ key: "projects", label: "项目列表", to: paths.projects(), disabled: false }],
    },
    {
      key: "project",
      label: "项目工作台",
  ...
  if (isAdmin) {
    sections.push({
      ...
      label: "系统管理",
      items: [
        { key: "users", label: "用户与权限管理", to: paths.projects("users"), disabled: false },
        { key: "graph", label: "图谱管理", to: paths.projects("graph"), disabled: false },
      ],
    });
  }

...
function renderFaultModes(element, data) {
	...
	svg
		...
		.attr("fill", "#eef1f5");
	svg
		.selectAll(".report-mode-bar")
		.data(data)
		.join("rect")
		.attr("x", margin.left)
		.attr("y", (item) => y(item.name))
		.attr("width", (item) => x(item.count) - margin.left)
		.attr("height", y.bandwidth())
		.attr("rx", 5)
		.attr("fill", COLORS.mode);
	svg
		.selectAll(".report-mode-value")
		.data(data)
		.join("text")
		.attr("class", "report-chart-value")
		.attr("x", width - margin.right + 8)
		.attr("y", (item) => y(item.name) + y.bandwidth() / 2 + 4)
		.text((item) => `${item.count} · ${Math.round(item.ratio * 100)}%`);
}
...
function renderPropagationPaths(element, data, onClick) {
	...
	rows
		...
		.attr("x", 0)
		.attr("y", 0)
		.attr("width", (item) => 5 + (105 * item.count) / maxCount)
		.attr("height", 52)
		.attr("rx", 8)
		.attr("fill", "#ebe8f7");
	rows
		.append("text")
		.attr("class", "report-path-rank")
		.attr("x", 17)
		.attr("y", 31)
		.text((_item, index) => String(index + 1).padStart(2, "0"));
	rows
		.append("text")
		.attr("class", "report-path-label")
		.attr("x", 68)
		.attr("y", 25)
		.text((item) => truncate(item.label, 72));
	rows
		.append("text")
		.attr("class", "report-path-meta")
		.attr("x", 68)
		.attr("y", 42)
		.text((item) => `${item.nodes.length} 个节点 · 占本批次 ${Math.round(item.ratio * 100)}%`);
	rows
		.append("text")
		.attr("class", "report-path-count")
		.attr("x", width - 22)
		.attr("y", 31)
		.attr("text-anchor", "end")
		.text((item) => `${item.count} 次`);
}

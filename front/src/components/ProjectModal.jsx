...
const statusOptions = [
	{
		value: "active",
		label: (
			<span className="select-option-with-icon">
			...
			<form onSubmit={submit}>
				...
				<div className="field" style={{ marginTop: 14 }}>
					...
					<TextArea
						...
						rows={3}
						placeholder="填写该项目的架构特点、核心模块或负责团队..."
						onChange={(description) => setValues((v) => ({ ...v, description }))}
					/>
				</div>
				{isEdit ? (
					<div className="field" style={{ marginTop: 14 }}>
						<label htmlFor="projectStatus">项目运行状态</label>
						<Select
							id="projectStatus"
							className="select"
							value={values.status}
				...
				>
					<Button theme="light" type="tertiary" className="button button-secondary" onClick={onClose}>
						取消
					</Button>
					<Button htmlType="submit" theme="solid" type="primary" className="button button-primary" loading={busy}>
						{isEdit ? "保存修改" : "立即创建项目"}
					</Button>
				</div>
			</form>

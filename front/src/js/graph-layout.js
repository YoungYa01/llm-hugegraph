...
function applyCollisions(points, nodeRadius, spacing = 24) {
  const minimumDistance = nodeRadius * 2 + spacing;
...
export function createForceLayout(nodes, edges, options = {}) {
  ...
  const linkDistance = Number(options.linkDistance) || 168;
  const collisionSpacing = Number(options.collisionSpacing) || 28;
  ...
  const step = () => {
    ...
    // Keep unrelated neighbours apart as well. The original test layout only
    // separated intersecting circles, so dense graphs converged into a ball and
    // made dependency directions impossible to follow.
    const repulsionDistance = linkDistance * 1.35;
    const repulsionCellSize = repulsionDistance;
    const repulsionGrid = new Map();
    points.forEach((point) => {
      const key = `${Math.floor(point.x / repulsionCellSize)}:${Math.floor(point.y / repulsionCellSize)}`;
      if (!repulsionGrid.has(key)) repulsionGrid.set(key, []);
      repulsionGrid.get(key).push(point);
    });
    points.forEach((point) => {
      const cellX = Math.floor(point.x / repulsionCellSize);
      const cellY = Math.floor(point.y / repulsionCellSize);
      for (let offsetX = -1; offsetX <= 1; offsetX += 1) {
        for (let offsetY = -1; offsetY <= 1; offsetY += 1) {
          (repulsionGrid.get(`${cellX + offsetX}:${cellY + offsetY}`) || []).forEach((other) => {
            if (point.name >= other.name) return;
            let deltaX = other.x - point.x;
            let deltaY = other.y - point.y;
            let distance = Math.hypot(deltaX, deltaY);
            ...
            const strength = ((repulsionDistance - distance) / repulsionDistance) * 0.7 * alpha;
            const forceX = (deltaX / distance) * strength;
            const forceY = (deltaY / distance) * strength;
            if (!Number.isFinite(point.fx)) {
              point.vx -= forceX;
              point.vy -= forceY;
            }
            if (!Number.isFinite(other.fx)) {
              other.vx += forceX;
              other.vy += forceY;
            }
          });
    ...
    maxMovement = Math.max(maxMovement, applyCollisions([...points.values()], nodeRadius, collisionSpacing));
    alpha *= 0.94;

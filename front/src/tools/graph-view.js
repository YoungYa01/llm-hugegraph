...
export function createSwimlanePlan(nodes, minimumWidth = 960) {
	...
	const laneSpecs = swimlanes.map((lane) => {
		...
		return {
			...
			nodes: laneNodes,
			columns,
			rows: Math.max(1, Math.ceil(laneNodes.length / columns)),
		};
	});
...
export function renderGraph(
	...
	if (isSwimlane) {
		const guides = guideLayer
			.selectAll("g.graph-swimlane")
	...
	const edgeGroups = baseEdgeLayer
		.selectAll("g.graph-edge-record")
	...
	const overlayUnderlays = primaryUnderlayLayer
		.selectAll("path")
		.data(primaryOverlayData)
		.join("path")
		.attr("class", "graph-propagation-underlay")
		.attr("fill", "none");
	const primaryOverlays = primaryLayer
		.selectAll("path")
		.data(primaryOverlayData)
		.join("path")
	...
	const alternativeOverlays = alternativeLayer
		.selectAll("path")
	...
	const nodeSelection = nodeLayer
		...
		.attr("class", (node) =>
			[
				...
				(node.name === semantics.start || node.name === semantics.end) && semantics.primary?.source === "llm"
					? "graph-node-primary-model"
					: "",
				(node.name === semantics.start || node.name === semantics.end) && semantics.primary?.source === "algorithm"
					? "graph-node-primary-algorithm"
					: "",
			]
				.filter(Boolean)
				.join(" "),
	...
	if (isSwimlane) {
		...
		simulation
			...
			.force(
				"collision",
				forceCollide(NODE_RADIUS + (mode === "incident" ? 34 : 30))
					.strength(0.95)
					.iterations(2),
			)
			...
			.force(
				"y",
				forceY(height / 2).strength((node) => (mode === "incident" && primaryIndexes.has(node.name) ? 0.22 : 0.024)),
			)
			.velocityDecay(0.34)
			.alphaDecay(0.035);
	}
	...
	const chooseEdge = (selectedLink) => {
		nodeSelection
			.classed(
				"graph-related",
				(node) => node.name === selectedLink.data.source || node.name === selectedLink.data.target,
			)
			.classed(
				"graph-muted",
				(node) => node.name !== selectedLink.data.source && node.name !== selectedLink.data.target,
	...
	const nodeDrag = drag()
		...
		.on("start", (event, node) => {
			...
			node.fy = node.y;
			if (isSwimlane) {
				guideLayer.selectAll(".graph-swimlane").classed("graph-swimlane-active", (lane) => lane.id === node.laneId);
			}
		})
		.on("drag", (event, node) => {

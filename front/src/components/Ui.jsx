...
export function Badge({ value, type = "status", className = "", children, style }) {
	...
	return (
		<Tag className={`badge badge-${value || "neutral"}${className ? ` ${className}` : ""}`} style={style}>
			{label}
		</Tag>
	);
}
...
export function LoadingState({ message = "正在加载…", minHeight }) {
	return (
		<div className="state-panel semi-state-panel" style={minHeight ? { minHeight } : undefined}>
			<Spin size="large" />
			<p>{message}</p>
		</div>
	);
...
export function EmptyState({ title, detail, children, compact = false }) {
	return (
		<div className={compact ? "empty-compact" : "empty-state"}>
			<Empty
				image={
					<span className="semi-empty-state-icon">
						<IconInfoCircle />
					</span>
				}
				title={title}
...
export function ErrorState({ error, onRetry }) {
	return (
		<div className="empty-state error-state">
			<Empty
				image={
					<span className="semi-empty-state-icon semi-empty-state-icon-error">
						<IconAlertTriangle />
					</span>
				}
				title="加载失败"
			...
			/>
			{onRetry ? (
				<AppButton icon={<IconRefresh />} className="button button-secondary" onClick={onRetry}>
					重新加载
				</AppButton>
			) : null}
		</div>
...
export function BusyButton({
	...
	return (
		<AppButton {...props} type={type} className={className} disabled={disabled || busy} loading={busy}>

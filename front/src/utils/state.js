...
function writeLogTasks(tasks) {
	...
	logTaskListeners.forEach((listener) => {
		listener(tasks);
	});
}
			...
			try {
				...
				if ((task.type === "analyze" && status === "completed") || status === "failed") {
					window.dispatchEvent(
						new CustomEvent("log-task:finished", {
							detail: { task: { ...task, batch, status } },
						}),
					);
					continue;
				}
				next.push({
					...task,
					status,
					summary: batch?.summary || task.summary || {},
					batch,
				...
				if (task.type === "delete" && error.status === 404) {
					window.dispatchEvent(
						new CustomEvent("log-task:finished", {
							detail: { task: { ...task, status: "completed" } },
						}),
					);
					continue;
				}

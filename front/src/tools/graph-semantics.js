...
const validHypothesis = (hypothesis) => hypothesis && Array.isArray(hypothesis.chain) && hypothesis.chain.length > 0;
...
export function buildIncidentSemantics(hypotheses = [], llmDecision = {}, visibleNames = null) {
	...
	const displayItems = Array.isArray(llmDecision.propagation_path)
		...
		: Array.isArray(llmDecision.display_chain)
			? llmDecision.display_chain
			: [];
	...
	const modelPrimary =
		isModelDecision && llmDecision.selected_node_id && modelChain.length
			? {
	...
	const primary = primaryCandidate
		? {
				...primaryCandidate,
				source: modelPrimary || selected ? "llm" : "algorithm",
			}
		: null;
	const chain = primary?.chain || [];
	const proposedDisplay = new Map(
		displayItems.filter((item) => item && typeof item === "object").map((item) => [String(item.node || ""), item]),
	);
	...
	const candidateRanks =
		primary?.source === "algorithm" && primary?.rank
			? {
	...
	const appendChain = (hypothesis, variant) => {
		const missing = visibleNames ? hypothesis.chain.filter((name) => !visibleNames.has(name)) : [];
		if (missing.length) {

...
function SectionHeading({ number, title, detail = "" }) {
  return <div className="batch-report-section-heading"><span>{number}</span><div><h2>{title}</h2><p>{detail}</p></div></div>;
}
...
function CountChip({ label, value, severity }) {
  return <span className={`batch-report-count-chip${severity ? ` count-${severity}` : ""}`}><b>{value}</b>{label}</span>;
}

function NodeTable({ nodes }) {
  if (!nodes.length) return null;
  const columns = [
    { title: "节点", dataIndex: "node", render: (value) => <strong>{value}</strong> },
    { title: "根因命中", dataIndex: "root_hits", render: (value) => <b className="report-number report-number-primary">{value || 0}</b> },
    { title: "传播链出现", dataIndex: "chain_hits", render: (value) => <b className="report-number">{value || 0}</b> },
    { title: "关联故障", dataIndex: "incident_count", render: (value) => value || 0 },
    { title: "根因占比", dataIndex: "root_ratio", render: (value) => `${Math.round(Number(value || 0) * 100)}%` },
    { title: "主要模式", dataIndex: "fault_modes", render: (items = []) => items.length ? <div className="report-mode-tags">{items.map((item) => <Tag key={item} color="blue">{item}</Tag>)}</div> : "—" },
    { title: "最近出现", dataIndex: "latest_incident_at", render: formatDate },
  ];
  return <div className="batch-report-table-scroll batch-report-node-table"><Table className="logscope-semi-table" columns={columns} dataSource={nodes} rowKey={(item) => item.node} pagination={false} /></div>;
}

function FaultModeList({ items }) {
  if (!items.length) return null;
  return <div className="batch-report-compact-list">{items.slice(0, 6).map((item, index) => (
    <div key={`${item.label}-${index}`}><strong>{item.label}</strong><span>主要根因：{item.top_root_node || "待确认"}</span><b>{item.count || 0} 次</b></div>
  ))}</div>;
}

function RecommendationList({ items }) {
  if (!items.length) return <EmptyState title="暂无治理建议" detail="需要先形成有效的 RCA 故障结论。" />;
  return <div className="batch-report-recommendations">{items.map((item, index) => (
    <article key={`${item.title}-${index}`}>
      <span className={`priority priority-${String(item.priority || "P2").toLowerCase()}`}>{item.priority || "P2"}</span>
      <div><h3>{item.title}</h3><p>{item.description}</p>{item.nodes?.length ? <small>涉及节点：{item.nodes.join("、")}</small> : null}</div>
    </article>
  ))}</div>;
}
...
function FocusNodeCards({ items, projectId }) {
  ...
  return <div className="batch-report-focus-grid">{items.map((item, index) => (
    <article key={`${item.node}-${index}`}>
      <div className="batch-report-focus-title"><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{item.node}</h3><p>{item.description}</p></div></div>
      <dl>
        <div><dt>根因命中</dt><dd>{item.root_hits || 0} 次</dd></div>
        <div><dt>传播链出现</dt><dd>{item.chain_hits || 0} 次</dd></div>
        <div><dt>关联故障</dt><dd>{item.incident_count || 0} 个</dd></div>
      </dl>
      <div className="batch-report-focus-tags">
        {(item.fault_modes || []).map((mode) => <span key={`m-${mode}`}>{mode}</span>)}
        {(item.affected_nodes || []).map((node) => <span className="affected" key={`a-${node}`}>影响 {node}</span>)}
      </div>
      {item.representative_evidence ? <blockquote><b>代表证据</b>{item.representative_evidence}</blockquote> : null}
      <div className="batch-report-incident-links">{(item.incident_ids || []).slice(0, 5).map((id) => (
        <Link key={id} to={paths.incident(projectId, id)}>查看故障 {String(id).slice(0, 8)}</Link>
      ))}</div>
    </article>
  ))}</div>;
}
...
export function LogReportPage({ project, batchId }) {
  ...
  const [state, setState] = useState({ loading: true, batch: null, report: null, error: null });
  ...
  useEffect(() => {
    ...
    renderReportCharts(
      { nodeFrequency: nodeRef.current, faultModes: modeRef.current, propagationPaths: pathRef.current },
      state.report,
      {
        onPathClick: (path) => {
  ...
  return (
    <div className="batch-report-page">
      <header className="batch-report-header">
        <div>
          <Link className="batch-report-back" to={paths.reports(project.id)}><IconChevronLeft size="small" />返回综合分析报告</Link>
          <div className="batch-report-kicker">BATCH RCA REPORT</div>
          <h1>综合诊断报告</h1>
          <p>汇总本批次已经完成根因分析的故障结论，定位高频故障节点、共性模式与治理优先级。</p>
        </div>
        <div className="page-actions">
          <AppButton icon={<IconRefresh />} className="button button-secondary" type="button" onClick={load}>刷新报告</AppButton>
          <Link className="button button-primary link-button-with-icon" to={paths.incidents(project.id, { batch: batchId })}><IconBarChartVStroked size="small" />查看关联故障</Link>
        </div>
      </header>
      ...
      <section className="batch-report-figures" aria-label="批次关键统计">
        ...
        <Figure label="根因节点" value={summary.root_node_count ?? 0} hint={`传播涉及 ${summary.node_count ?? 0} 个节点`} />
        <Figure label="平均置信度" value={formatConfidence(summary.average_confidence)} hint="全部 RCA 结论平均值" />
        <Figure label="治理闭环" value={`${summary.resolved_count ?? 0} / ${summary.incident_count ?? 0}`} hint={`${Math.round(Number(summary.resolution_rate || 0) * 100)}% 已解决`} />
      </section>
      ...
      <section className="batch-report-section batch-report-conclusion">
        ...
        {(report.executive_conclusions || []).length ? (
          <ol className="batch-report-conclusion-list">{report.executive_conclusions.map((item, index) => <li key={index}>{item}</li>)}</ol>
      ...
      <section className="batch-report-section">
        ...
        <div className="batch-report-legend"><span><i className="legend-root" />根因命中</span><span><i className="legend-chain" />传播链出现</span></div>
      ...
      <section className="batch-report-section">
        <SectionHeading number="06" title="高频故障传播路径" detail="按完整传播链聚合排名；点击路径可查看其代表故障。" />

...
function CurrentPage({ section, project, batchId, incidentId, onProjectUpdated }) {
	switch (section) {
		case "architecture":
			return <ArchitecturePage project={project} />;
		case "logs":
			return <LogsPage project={project} />;
		case "reports":
			return <ReportsPage project={project} />;
		case "log-report":
			return <LogReportPage project={project} batchId={batchId} />;
		case "incidents":
			return <IncidentsPage project={project} />;
		case "incident-detail":
			return <IncidentDetailPage project={project} incidentId={incidentId} />;
		// case "overview":
		default:
			return <OverviewPage project={project} onProjectUpdated={onProjectUpdated} />;
	}
}
...
export function ProjectPage({ section, onLogout, account, onAccountUpdated }) {
	...
	useEffect(() => {
		...
		loadProject(projectId)
			...
			.catch((error) => {
				if (active) setState({ loading: false, project: null, error });
			});
		return () => {
			active = false;
		};
	...
	if (state.error || !state.project) {
		return (
			<main className="content">
				<ErrorState error={state.error || new Error("项目不存在")} onRetry={() => navigate("/projects")} />
			</main>
		);
	}
